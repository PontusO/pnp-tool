...
import claudepnp as cli
from optimizer import PnPOptimizer, PackageRule, TimingConfig
...
class TestCleanField:
    def test_european_decimal_comma_converted(self):
        assert cli._clean_field("4,7uF") == "4.7uF"

    def test_leading_decimal_comma_converted(self):
        assert cli._clean_field("0,1uF") == "0.1uF"

    def test_part_number_comma_replaced_with_underscore(self):
        assert cli._clean_field("74AVC4TD245BQ,115") == "74AVC4TD245BQ_115"

    def test_semicolon_replaced_with_underscore(self):
        assert cli._clean_field("some;value") == "some_value"

    def test_plain_value_unchanged(self):
        assert cli._clean_field("100nF") == "100nF"

    def test_empty_string_unchanged(self):
        assert cli._clean_field("") == ""

    def test_multiple_decimal_commas(self):
        result = cli._clean_field("1,2,3")
...
class TestDetectCsvDelimiter:
    def test_comma_detected(self, tmp_path):
        f = tmp_path / "a.csv"
        f.write_text("a,b,c\n1,2,3\n")
        assert cli._detect_csv_delimiter(f) == ","
...
class TestLoadBom:
    def test_csv_loads_correct_count(self, simple_bom):
        assert len(cli.load_bom(simple_bom)) == 8

    def test_dnm_components_skipped_by_default(self, dnm_bom):
        placements = cli.load_bom(dnm_bom)
        refdes = [p.refdes for p in placements]
        assert "R1" in refdes
        assert "R2" not in refdes
        assert "C1" not in refdes

    def test_dnm_components_included_with_flag(self, dnm_bom):
        assert len(cli.load_bom(dnm_bom, include_dnm=True)) == 3

    def test_space_separated_txt_loads(self):
        assert len(cli.load_bom(REPO_ROOT / "nr52-top.txt")) > 0
...
class TestMatchRule:
    ...
    def test_exact_match(self):
        assert PnPOptimizer.match_rule("R0402", self._rule("R0402", "exact"))

    def test_exact_no_partial(self):
        assert not PnPOptimizer.match_rule("R0402X", self._rule("R0402", "exact"))

    def test_exact_case_insensitive(self):
        assert PnPOptimizer.match_rule("r0402", self._rule("R0402", "exact"))

    def test_prefix_match(self):
        assert PnPOptimizer.match_rule("C0402X55", self._rule("C0402", "prefix"))

    def test_prefix_no_wrong_start(self):
        assert not PnPOptimizer.match_rule("XC0402", self._rule("C0402", "prefix"))

    def test_contains_match(self):
        assert PnPOptimizer.match_rule("BGA_64", self._rule("BGA", "contains"))

    def test_contains_no_wrong_string(self):
        assert not PnPOptimizer.match_rule("ABCD", self._rule("BGA", "contains"))

    def test_regex_match(self):
        assert PnPOptimizer.match_rule("QFN50P200X200X100-8N", self._rule(r"QFN\d+P\d+", "regex"))

    def test_regex_no_match(self):
        assert not PnPOptimizer.match_rule("SOIC8", self._rule(r"QFN\d+", "regex"))
...
class TestLoadPackageRules:
    def test_loads_without_error(self):
        rules = cli.load_package_rules(REPO_ROOT / "package_rules.csv")
        assert len(rules) > 0
...
class TestLoadTimingConfig:
    def test_loads_from_file(self):
        cfg = cli.load_timing_config(REPO_ROOT / "timing_config.csv")
    ...
    def test_missing_file_returns_defaults(self, tmp_path):
        cfg = cli.load_timing_config(tmp_path / "nonexistent.csv")
        assert cfg.front_time_min == pytest.approx(0.5)
        assert cfg.rear_time_max  == pytest.approx(1.2)
